  ...
  function renderCampoData(label, valore, valoreObj, setValore, setValoreObj, mostra, setMostra) {
    ...
    return (
      <>
        ...
        >
          <Text style={[styles.dataBtnText, !valore && { color: "#555" }]}>
    ...
    return (
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={styles.input} placeholder="es. Serata Jazz" placeholderTextColor="#555" value={titoloLocale} onChangeText={setTitoloLocale} />

          <Text style={styles.label}>DESCRIZIONE</Text>
          <TextInput style={[styles.input, styles.inputMultiline]} placeholder="Descrivi l'evento..." placeholderTextColor="#555" value={descrizioneLocale} onChangeText={setDescrizioneLocale} multiline numberOfLines={4} />
          ...
          <TextInput style={styles.input} placeholder="es. 21:00" placeholderTextColor="#555" value={oraLocale} onChangeText={setOraLocale} keyboardType="numeric" />

          <Text style={styles.label}>PREZZO (€)</Text>
          <TextInput style={styles.input} placeholder="es. 10 (0 se gratuito)" placeholderTextColor="#555" value={prezzoLocale} onChangeText={setPrezzoLocale} keyboardType="numeric" />

          <Text style={styles.label}>MAX PARTECIPANTI</Text>
          <TextInput style={styles.input} placeholder="es. 100" placeholderTextColor="#555" value={maxPartecipantiLocale} onChangeText={setMaxPartecipantiLocale} keyboardType="numeric" />
          ...
          <TouchableOpacity style={styles.button} onPress={handleCreaEvento} disabled={loadingLocale}>
            {loadingLocale ? <ActivityIndicator color="#0a0a0a" /> : <Text style={styles.buttonText}>CREA EVENTO</Text>}
          </TouchableOpacity>
        </ScrollView>
      </KeyboardAvoidingView>
    );
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.label}>TITOLO EVENTO</Text>
        <TextInput style={styles.input} placeholder="es. Festa di compleanno" placeholderTextColor="#555" value={titolo} onChangeText={setTitolo} />

        {renderCampoData("DATA", data, dataObj, setData, setDataObj, mostraCalendario, setMostraCalendario)}

        <Text style={styles.label}>ORA (HH:MM)</Text>
        <TextInput style={styles.input} placeholder="es. 21:00" placeholderTextColor="#555" value={ora} onChangeText={setOra} keyboardType="numeric" />

        <Text style={styles.label}>NUMERO PARTECIPANTI</Text>
        <TextInput style={styles.input} placeholder="es. 50" placeholderTextColor="#555" value={numPartecipanti} onChangeText={setNumPartecipanti} keyboardType="numeric" />

        <Text style={styles.label}>MESSAGGIO AL LOCALE (opzionale)</Text>
        <TextInput style={[styles.input, styles.inputMultiline]} placeholder="Descrivi il tuo evento..." placeholderTextColor="#555" value={messaggio2} onChangeText={setMessaggio2} multiline numberOfLines={4} />
        ...
        <TouchableOpacity style={styles.button} onPress={handleInviaRichiesta} disabled={loading}>
          {loading ? <ActivityIndicator color="#0a0a0a" /> : <Text style={styles.buttonText}>INVIA RICHIESTA</Text>}
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  backText: { color: "#c9b99a", fontSize: 10, letterSpacing: 3 },
  titoloPagina: { color: "#ffffff", fontSize: 24, fontWeight: "200", letterSpacing: 8, marginBottom: 12 },
  linea: { height: 1, backgroundColor: "#1a1a1a", marginVertical: 16 },
  subtitle: { color: "#555", fontSize: 12, lineHeight: 18, marginBottom: 24 },
  label: { color: "#c9b99a", fontSize: 10, letterSpacing: 3, marginBottom: 8, marginTop: 16 },
  input: { borderBottomWidth: 1, borderBottomColor: "#333", color: "#ffffff", fontSize: 15, paddingVertical: 12, paddingHorizontal: 4 },
  inputMultiline: { borderWidth: 1, borderColor: "#333", padding: 12, height: 100, textAlignVertical: "top" },
  dataBtn: { borderBottomWidth: 1, borderBottomColor: "#333", paddingVertical: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  dataBtnText: { color: "#ffffff", fontSize: 14, fontWeight: "200" },
  dataIcon: { fontSize: 16 },
  pickerBtn: { borderWidth: 1, borderColor: "#333", padding: 14, flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 8 },
  pickerBtnText: { color: "#ffffff", fontSize: 14, fontWeight: "200" },
  pickerArrow: { color: "#c9b99a", fontSize: 12 },
  pickerDropdown: { borderWidth: 1, borderColor: "#333", borderTopWidth: 0, backgroundColor: "#0f0f0f", maxHeight: 200 },
  pickerSearch: { borderBottomWidth: 1, borderBottomColor: "#333", color: "#ffffff", padding: 12, fontSize: 13 },
  pickerItem: { padding: 14, borderBottomWidth: 1, borderBottomColor: "#1a1a1a" },
  pickerItemText: { color: "#ffffff", fontSize: 13, fontWeight: "300" },
  pickerItemSub: { color: "#555", fontSize: 11, marginTop: 2 },
  pickerEmpty: { color: "#555", padding: 16, textAlign: "center", fontSize: 12 },
  error: { color: "#e07070", fontSize: 12, textAlign: "center", marginTop: 16 },
  success: { color: "#c9b99a", fontSize: 12, textAlign: "center", marginTop: 16, letterSpacing: 1 },
  button: { backgroundColor: "#c9b99a", paddingVertical: 16, alignItems: "center", marginTop: 36, marginBottom: 24 },
  buttonText: { color: "#0a0a0a", fontSize: 12, fontWeight: "700", letterSpacing: 4 },
});